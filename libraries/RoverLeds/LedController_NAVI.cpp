...
void LedController_NAVI::runLedController()
{
	...
	switch(this->_currentUniversalLEDMode)
	{
		...
		case LED_NIGHT_TIME_MODE:
			//Do thing recurringly just in case rover motion went back to LED_MOTION_STANDARD, then this will re-setup the night time lights
...

	{
		...
		switch(this->_currentRoverMotion)
		{
			...
			case LED_MOTION_TURN_LEFT:
					...
					{
						case 0:
							//====500ms LED Code  = 4x125ms(500ms delay) x 2 patterns = 8 cases of 1 sec duration (8x125ms = 1 sec)		
							
							//Left Turn Signal Headlight - Turn Off
							...
							//Right Turn Signal Headlight - Turn Off
							//Do Nothing
							...
							//====125ms LED Code = (1x125ms(125ms delay) x 4 patterns) x 2 repeated = 8 cases of 1 sec duration (8x125ms = 1 sec)
						...
						case 4:
							...
							//Left Turn Signal Headlight - Turn On
							for(byte i = 0; i < sizeof(this->_LED_NAMES_For_MotionTurnLeft_Headlight) / sizeof(this->_LED_NAMES_For_MotionTurnLeft_Headlight[0]) ; i++)
							{
			...
			case LED_MOTION_TURN_RIGHT:
					...
					switch(this->_roverMotionPatternIndexCounter)
					{
						...
						case 4:
							...
							//Right Turn Signal Headlight - Turn On
							for(byte i = 0; i < sizeof(this->_LED_NAMES_For_MotionTurnRight_Headlight) / sizeof(this->_LED_NAMES_For_MotionTurnRight_Headlight[0]) ; i++)
							{
			...
			case LED_MOTION_BRAKE:
					...
					if (this->_counterPtr->countReached())
					{
						...
						if(this->_roverMotionPatternIndexCounter < 8) //At the last pattern index, don't increment/reset. Hold the brake tail lights on. So keep incrementing the pattern counter unless it's the state where the tail light should hold it's value, then don't increment it anymore. As you don't want it to reset. 
	...
	{
		switch(this->_currentIRBeaconState)
		{
...
void LedController_NAVI::setHazardLightsMode(byte desiredHazardLightsState)
{
	...
	*/
	...
	//Change delay duration and then reset the counter
	//Note: The setHazardLightsMode() default is 500ms.
	this->_counterPtr->setStopValue(this->_periodsForLongDelay);
	//reset the delay counter
	this->_counterPtr->counterReset();
...
void LedController_NAVI::setUnderglowLightMode(byte desiredUnderglowLightState)
{
	//Assign the underglow light mode if it's one of the valid modes
	switch(desiredUnderglowLightState)
	{
		case LED_UNDERGLOW_OFF:
			this->_currentUnderglowState = desiredUnderglowLightState;
			break;
		case LED_UNDERGLOW_ON:
			this->_currentUnderglowState = desiredUnderglowLightState;
			break;
...
void LedController_NAVI::setIRBeaconLightMode(byte desiredIrBeaconLightState)
{
	//Assign the IR Beacon light mode if it's one of the valid modes
	switch(desiredIrBeaconLightState)
	{
		case LED_IR_BEACON_ALL_OFF:
			this->_currentIRBeaconState = desiredIrBeaconLightState;
			break;
		case LED_IR_BEACON_ALL_ON:
			this->_currentIRBeaconState = desiredIrBeaconLightState;
			break;
...
void LedController_NAVI::setBlueBeaconLightMode(byte desiredBlueBeaconLightState)
{
	//Assign the Blue Beacon light mode if it's one of the valid modes
	switch(desiredBlueBeaconLightState)
	{
		case LED_BLUE_BEACON_ALL_OFF:
			this->_currentBlueBeaconState = desiredBlueBeaconLightState;
			break;
		case LED_BLUE_BEACON_ALL_ON:
			this->_currentBlueBeaconState = desiredBlueBeaconLightState;
			break;
...
void LedController_NAVI::setRoverMotion(byte desiredRoverMotion)
{
	...
	*/
	...
	//Change delay duration and then reset the counter
	//Note: The roverMotion() default is 125ms.
	this->_counterPtr->setStopValue(this->_periodsForShortDelay);
	//reset the delay counter
	this->_counterPtr->counterReset();
	//initialize the current pattern index for  rover motion	
	...
	switch(desiredRoverMotion)
	{
		...
		case LED_MOTION_TURN_LEFT:
			...
			//Set Custom Delay (if required)
			//This mode uses a 125ms delay. And that's the default delay for setRoverMotion().
